 ...
 * @author evand
 */
public class MfJpaClassCustomization extends MfClassCustomization {
    ...
    private void forAllClasses(List<ClassMetadata> classMetadataList) {
        for (ClassMetadata clazz : classMetadataList) {
            ...
            clazz.getAnnotations().add(new Annotation("@Entity"));
        }
    }
        ...
        for (ClassMetadata clazz : classMetadataList) {
            for (ClassField clazzField : clazz.getFields()) {
                if (clazzField.getRelationshipType() == ClassRelationshipFieldType.ARRAY_OF_OBJECTS){
                    ...
                    clazzField.getAnnotations().add(new Annotation("@OneToMany(mappedBy = \""+clazzField.getRefForeignFieldName()+"\", fetch = FetchType.EAGER)"));
